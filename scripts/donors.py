...
def add_donor(conn, args):
    ...
    conn.execute(
        """INSERT INTO nonprofitclaw_donor
           (id, naming_series, donor_type, name, email, phone, address,
            city, state, zip_code, tax_id, donor_level, notes, is_active, company_id)
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (
            donor_id, naming, donor_type, name,
    ...
    return ok({"id": donor_id, "naming_series": naming, "name": name})
...
def update_donor(conn, args):
    ...
    row = conn.execute("SELECT id, company_id FROM nonprofitclaw_donor WHERE id=?", (donor_id,)).fetchone()
    if not row:
    ...
    for col, attr in [
        ...
        ("zip_code", "zip_code"), ("tax_id", "tax_id"),
    ...
    conn.execute(f"UPDATE nonprofitclaw_donor SET {', '.join(fields)} WHERE id=?", values)
    conn.commit()
...
def list_donors(conn, args):
    ...
    total = conn.execute(
        f"SELECT COUNT(*) FROM nonprofitclaw_donor WHERE {where_sql}", params
    ).fetchone()[0]

    rows = conn.execute(
        f"""SELECT id, naming_series, donor_type, name, email, phone,
                   total_donated, donation_count, donor_level, is_active
            FROM nonprofitclaw_donor WHERE {where_sql}
...
def get_donor(conn, args):
    ...
    row = conn.execute("SELECT * FROM nonprofitclaw_donor WHERE id=?", (donor_id,)).fetchone()
    if not row:
        return err(f"Donor {donor_id} not found")
...
def donor_giving_history(conn, args):
    ...
    row = conn.execute("SELECT id, name FROM nonprofitclaw_donor WHERE id=?", (donor_id,)).fetchone()
    if not row:
        return err(f"Donor {donor_id} not found")
...
def merge_donors(conn, args):
    ...
    source = conn.execute("SELECT id, company_id FROM nonprofitclaw_donor WHERE id=?", (source_id,)).fetchone()
    target = conn.execute("SELECT id, company_id FROM nonprofitclaw_donor WHERE id=?", (target_id,)).fetchone()
    if not source:
    ...
    try:
        ...
        conn.execute(
            """UPDATE nonprofitclaw_donor SET
        ...
        # Delete source donor
        conn.execute("DELETE FROM nonprofitclaw_donor WHERE id=?", (source_id,))
        conn.commit()
...
def import_donors(conn, args):
    return ok({"message": "Donor import is available via CSV upload. Use the web interface or provide a CSV file path.", "imported": 0})
...
def add_donation(conn, args):
    ...
    return ok({"id": donation_id, "naming_series": naming, "amount": str(amount)})
...
def refund_donation(conn, args):
    ...
    return ok({"id": donation_id, "refunded": True, "amount": str(amount)})
